...
void dfs(int node, int par,vector<int>& parent, vector<vector<int>>& g){
     

   for(auto child: g[node]){
      if(child!= par){
         parent[child]=node;
         dfs(child,node,parent,g);
      }
...
void DFS(int node,int par, int gpar,vector<vector<int>>& g,vector<int>& parent,string& s,vector<int>& a){
     ...
     if(par>=1 && gpar>=1){
      ...
      if(a[gpar]==a[node] && par!=gpar && gpar!=node){
         s[a[node]]='1';
      }
     ...
     for(auto child: g[node]){
      if(child!=par){
         DFS(child,node,parent[node],g,parent,s,a);
      }
     }
