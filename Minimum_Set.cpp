...
int bridge(int l1,int r1,int l2, int r2){
    int ans = 0;
    int start_bit = (l2 > 0) ? bitc(l2) : -1;

    for (int b = start_bit; b >= 0; --b) {
        int p = 1LL << b;
        int l1_bit = (l1 >> b) & 1;
        int r1_bit = (r1 >> b) & 1;
        int l2_bit = (l2 >> b) & 1;
        ...
        if (l1_bit != l2_bit) {
            ...
            bool r1_spans = (l1_bit == 0 && r1 >= (l1 | p));
            bool r2_spans = (l2_bit == 0 && r2 >= (l2 | p));

            if (!r1_spans && !r2_spans) {
                ans |= p;
            }
        }
    }
...
int rec(int start,int end,map<pair<int,int>,int>& dp){
    if (start >= end) {
        return 0;
    }
    ...
    long long result;
    if (start >= m) {
        result = rec(start - m, end - m, dp);
    } else {
        result = rec(start, m - 1, dp) + rec(m, end, dp) + bridge(start, m - 1, m, end);
    }
...
void solve(){
    ...
    // Need to create a MST
    // with edge wt = i^j for node i and j

    map<pair<int,int>,int> dp;

    cout<<rec(1,n,dp)<<endl;
