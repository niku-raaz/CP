...
double rec(int i,int ct,vector<double>& p,vector<vector<double>>& dp){
    ...
    double x=rec(i+1,ct+1,p,dp)*p[i];

    double y=rec(i+1,ct,p,dp)*(1-p[i]);
...
void solve(){
 ...
 //   cout<<setprecision(10)<<rec(0,0,p,dp);
    ...
    // dp[i][j] = propability if getting j heads till ith index
