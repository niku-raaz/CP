...
long long xorPairProduct(const vector<int>& arr) {
    const int MOD = 1e9 + 7;
    const int BITS = 32;
    long long result = 1;

    for (int i = 0; i < BITS; ++i) {
        long long cnt_1 = 0, cnt_0 = 0;
        for (int num : arr) {
            if (num & (1 << i)) cnt_1++;
            else cnt_0++;
        }
        // Contribution of the current bit to the product
        long long bitContribution = (cnt_1 * cnt_0) ;
        result = (result * (bitContribution + 1)) ; // Adding 1 to include "no contribution" as a valid case
    }
