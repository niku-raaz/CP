...
// can know ind by k-z;  // k is total bound

int rec(int right,int left,vector<int>& a){
    ...
    if(ind+1<n && tot<k)
        ans=max(ans,rec(right+1,left,a)+a[ind+1]);

    // can also go left

    if(ind>0 && left<z){
        ans=max(ans,rec(right,left+1,a)+a[ind-1]);
    }
