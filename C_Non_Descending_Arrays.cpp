...
int rec(int ind,int inda,int indb,int f,vector<int>& a,vector<int>& b){
    int n=a.size();
    ...
    int ans=0;
    if(dp[ind][inda][indb][f]!=-1){
        return dp[ind][inda][indb][f];
    }
    ...
    if(f==0){
        ...
        if(a[inda]<=a[ind] && b[indb]<=b[ind]){
            ans+=rec(ind+1,ind,ind,0,a,b);
            ans%=mod;
        }
        if(a[inda]<=b[ind] && b[indb]<=a[ind]){
            ans+=rec(ind+1,ind,ind,1,a,b);
             ans%=mod;
        }

    }else{
        // swpad
        if(a[indb]<=a[ind] && b[inda]<=b[ind]){
            ans+=rec(ind+1,ind,ind,0,a,b);
             ans%=mod;
        }
        if(a[indb]<=b[ind] && b[inda]<=a[ind]){
            ans+=rec(ind+1,ind,ind,1,a,b);
             ans%=mod;
        }
