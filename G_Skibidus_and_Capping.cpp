...
void createSemiPrimeSieve(int n)
{
    int v[n + 1];
...
void solve(){
    ...
    for(auto it: fsp){
        int pq=it.first;
        vector<int> fac=prime_factors(pq);
        if(fac.size()==1){
            int p=fac[0];
            int q=p;
            ans+=(mp[pq]*mp[p]+mp[pq]*mp[q]);
           // ans+=(mp[pq]*(mp[pq]+1))/2;
        }else{
            int p=fac[0];
            int q=fac[1];
...
int32_t main(){
  ...
  SieveOfEratosthenes(200003);
   createSemiPrimeSieve(200003);
