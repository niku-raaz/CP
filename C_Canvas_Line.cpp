  ...
  //  show(pins);
    ...
    for(int i=0;i<p;i++){
       ...
       // cout<<ans<<" ";pl;
        ...
        if(v[ans].first<=pp && v[ans].second>=pp){
            count[ans]++;
        }
        if(v[ans].first==pp && ans-1>=0){
            if(v[ans-1].second==pp){
                count[ans-1]++;
            }
        }

        if(v[ans].second==pp && ans+1<n){
            if(v[ans+1].first==pp){
                count[ans+1]++;
            }
        }
   ...
   // pl;
    ...
    for(int i=0;i<n;i++){
        ...
        if(i+1<n){
          int nextneed=2-count[i+1];
          //cout<<need<<" "<<nextneed;pl;
          
          if(nextneed>0 && need>0 && v[i].second==v[i+1].first && mp.find(v[i].second)==mp.end()){
            // both connected 
            // and both need
            // also check pins should not already exist at this ponit
            mp[v[i].second]++;
            ans+=1;
            count[i]+=1;
            count[i+1]+=1;
          ...
          if(need>0){
             //cout<<"mid mein \n";
             if(need==1){
                for(int num=v[i].first+1;num<v[i].second;num++){
