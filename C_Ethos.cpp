...
void solve(){
    ...
    // Now i go the root, need to make it a tree
    ...
    while(!q.empty()){
        ...
        for(auto neg: g[node]){
            ...
            // the edge is node-neg

            if(vis[neg]){
                // already visited means it should be removed
            }else{
                // its fine
                q.push(neg);
                vis[neg]=1;
