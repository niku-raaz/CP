...
void solve(){
   ...
   auto assignDFS=[&](int node,int ct,auto &&assignDFS)->int{
      vis[node]=1;
      //cout<<node;pl;
      for(auto& child: g[node]){
         if(vis[child]){
            return ans[node]=ct+1;
         }
         return ans[node]=assignDFS(child,ct+1,assignDFS);
      }
   };

   for(auto& start: rep){
      assignDFS(start,0,assignDFS);
   }
   //show(ans);

   auto calc=[&](int node,auto &&calc){
      if(ans[node]!=-1){
         return;
      }
      for(auto& child: g[node]){
