...
void dfs(int node, int par,vector<vector<int>>& g,vector<int>& vis){

    vis[node]=1;
    for(auto child: g[node]){
        if(child!=par && !vis[child]){
            dfs(child,node,g,vis);
        }
    }
...
void solve(){
    ...
    for(int i=0;i<n;i++){
        ...
        nodes.push_back(vp[i].first+M*vp[i].second);
        for(int j=0;j<n;j++){
            if(i!=j){
                ...
                if(vp[i].first==vp[j].first || vp[j].second==vp[i].second){
                    ...
                    int a=vp[i].first+M*vp[i].second;
                    int b=vp[j].first+M*vp[j].second;
                    g[a].pb(b);
                    g[b].pb(a);
                }
    ...
    // count number of componetnts in graph
