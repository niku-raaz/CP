...
bool check(const vector<int>& b, int k, int n) {
    ...
    int min_prefix = INT_MAX; // for p[0]
    
    for (int j = k; j <= n; ++j) {
 
        min_prefix = min(min_prefix, p[j - k]);
...
pair<int,int> check22(const vector<int>& b, int k, int n) {
    // b is the array of 1s and -1s
    vector<int> p(n + 1, 0);
    for (int i = 0; i < n; ++i) {
        p[i+1] = p[i] + b[i];
    }
    ...
    for (int j = k; j <= n; ++j) {
       ...
       // min_prefix = min(min_prefix, p[j - k]);
        ...
        if (p[j] >= min_prefix) {
            return {lo,j}; // Found a valid subarray
        }
    }
...
void solve(){
    ...
    while(lo<=hi){
        ...
        // Can we find mid as a median 
        // for a subarray k
        // i.e 
        // I wanna take >=k lenth subrray
        // where k/2 is <=mid
        // and k/2 >=mid
        vector<int> b(n);
        for(int i=0;i<n;i++){
      ...
      //  py;
        ...
        // take >=k length subarray st that its sum >=0
        // 
    ...
    pair<int,int> rr=check22(b,k,n);

    cout<<rr.first<<" "<<rr.second;pl;
