...
vector<vector<int>> g(200003);
   
bool sbs(const pair<int,int>&a,const pair<int,int>&b ){


    //  a.first=ind a.secod is depth of a.first

    if(a.second==b.second){

        if(g[a.first].size()<=g[b.first].size()){
            return 0;
        }else{
            return 1;
        }
...
void dfs(int node, int par, vector<vector<int>>& g,vector<int>& depth,vector<int>& subsize){
    ...
    for(auto child: g[node]){
        ...
        dfs(child,node,g,depth,subsize);
