...
void DFS_calc(int node,int par,vector<vector<int>>& g,vector<int>& ans1,vector<int>& ans2,vector<int>& a){
    
    if(node!=1){
        ans1[node]=max(a[node],a[node]-ans2[par]);
        ans2[node]=min(a[node],a[node]-ans1[par]);
    }
    ...
    for(auto child: g[node]){
        ...
        DFS_calc(child,node,g,ans1,ans2,a);
