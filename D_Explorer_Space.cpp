...
int rec(int i, int j, int k,vector<vector<vector<int>>>& dp, vector<vector<int>>& right, vector<vector<int>>& down){
        ...
        if(issafe(i-1,j)){
          ...
          ans=min(ans,rec(i-1,j,k-1,dp,right,down)+down[i-1][j]);

        }

        if(issafe(i+1,j)){
          // niche
          ans=min(ans,rec(i+1,j,k-1,dp,right,down)+down[i][j]);
        }

        if(issafe(i,j-1)){
          // 
          ans=min(ans,rec(i,j-1,k-1,dp,right,down)+right[i][j-1]);
        }

        if(issafe(i,j+1)){
          ans=min(ans,rec(i,j+1,k-1,dp,right,down)+right[i][j]);
        }
...
void solve(){
   ...
   for(int i=1;i<=n-1;i++){
    ...
    for(int i=1;i<=n;i++){
      for(int j=1;j<=m;j++){
        cout<<2*rec(i,j,k,dp,right,down)<<" ";
      }
      pl;
    }
