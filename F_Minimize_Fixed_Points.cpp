...
void solve(){
    ...
    // gcd(pi,i) >1 for all 2...n

    // Find good permutation with minimum fixed points
    ...
    // Now this a is good but we need to minimise the fixe points.
    // Currently its max

   // if a[i] is prime and its multiple are there . that can be handled
   // if a[i] is not prime (handled)
   // if a[i] is prime and it doesnt have have any multiple , we cant do anything
    ...
    for(int i=n;i>=2;i--){
        if(vis[i]==0){
            if(prime[i]){
                // check its multiple exist or not
                int x=n/i;
                if(x==1){
                    // does not have multiple so cant do anything
                    // So mark it visited
                    vis[i]=1;
                }else{
                    // it got multiples
                    // so slided all its unvisited multiple 
                    // develop a logic for it then

                    // we have total x multiples of it
                    ...
                    while(1){
                        // curr is unvisited
                        while(vis[next] && next<=n){
                              next+=i;
                        }
                        if(next>n){
                            break;
                        }
                        // now i got unvisited next
                        swap(a[curr],a[next]);
                        vis[curr]=1;
                        vis[next]=1;
                        curr=next;
