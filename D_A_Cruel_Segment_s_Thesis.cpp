        ...
        if(!vis[i]){
            ...
            if(s.size()==1){
               ...
               // cout<<p.first<<" "<<p.second;pl;
                ...
                ans+=max((right-v[i].first),(v[i].second-left));

                // delete from s


                s.erase(s.find({v[idx].second,idx}));
                s.erase(s.find({v[i].second,i}));
               // cout<<i<<" "<<idx<<" "<<s.size();pl;
