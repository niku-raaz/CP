...
void solve(){
    ...
    dp[1][0]=cold[a[1]]; // fisrt index ka answer with 0(non-existing) being the last process on another cpu
    // thanks gaurish
    ...
    for(int i=2;i<=n;i++){
        for(int j=0;j<=k;j++){  
            ...
            dp[i][a[i-1]]=min(dp[i][a[i-1]],dp[i-1][j]+(j==a[i]? hot[a[i]]:cold[a[i]]));
