...
// in DP check its subarray / subsequence / subset  
 ...
 const int MAX_MASKS = 1 << MAX_BITS;  // Upper bound of possible bitmasks (~35960 in practice)
 
 int F(vector<int>& arr) {
     unordered_map<int, int> freq;  // Frequency of each bitmask
 
     // Step 1: Compute bitmask frequency
     for (int num : arr) {
         int mask = 0;
     ...
     vector<int> masks;
     for (auto &[mask, _] : freq) {
         masks.push_back(mask);
     }
 
     long long count = 0;
     
     // Step 2: Use subset enumeration to find valid (AND == 0) pairs efficiently
     int m = masks.size();
     for (int i = 0; i < m; i++) {
         int mask1 = masks[i];
         count += (1LL * freq[mask1] * (freq[mask1] - 1)) / 2;  // Count pairs within the same mask
         
         // Enumerate all potential masks that can be ANDed with `mask1`
         for (int mask2 : masks) {
             if (mask1 < mask2 && (mask1 & mask2) == 0) {
                 count += 1LL * freq[mask1] * freq[mask2];
             }
         }
     }
