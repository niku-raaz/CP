...
void solve(){
    // 1 replcement
    // and then 1 Rectangle

    // Good Question
    // First lets forget about replacement
    // Then can we BS ?
    // No ig

    // we can do it in O(n) . min max tracking
    // can we do min max tracking for top 2 mosters? 
    ...
    int ans=(xmax1-xmin1+1)*(ymax1-ymin1+1);
    ...
    for(int i=0;i<n;i++){
        ...
        // Just calculate 
        // If we replace the current {x,y} with some other
        // Can do max O(logN) operation. or O(1) operation.
