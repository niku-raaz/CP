...
void solve(){
    ...
    vector<vector<int>> dp(n+1,vector<int>(k+1,MX));
    ...
    // dp[i][j]==min time to all people from i to n-1 to pic keys and go to offices
    ...
    for(int i=n-1;i>=0;i--){
        for(int j=k-1;j>=0;j--){
            ...
            int take=max((abs(a[i]-b[j])+abs(b[j]-p)),dp[i+1][j+1]);
