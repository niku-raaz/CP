...
// in DP check its subarray / subsequence / subset  
 ...
 void dfs(int node,int par,vector<vector<int>>& g,vector<int>& dp,vector<int>& a){
    bool leaf=0;
    if(node!=0 && g[node].size()==1){
        // leaf node
        leaf=1;
        dp[node]=a[node];
       // cout<<" called "<<node<<" "<<dp[node];pl;;
    }
    ...
    for(auto child: g[node]){
      ...
      //  cout<<node<<" n "<<child<<" c "<<dp[child]<<" ";pl;
        dfs(child,node,g,dp,a);
     //   cout<<node<<" n "<<child<<" c "<<dp[child]<<" ";pl;
        mn=min(mn,dp[child]);
    }
    ...
    if(a[node]<mn && !leaf){
       // cout<<node<<" "<<a[node]<<" "<<mn;pl;
        int diff=(mn-a[node])/2;
