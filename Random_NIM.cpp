...
int rec(int ind,int lastwin,int last_move,vector<int>& a,vector<vector<int>>& dp){
    if(ind==a.size()){
        if(lastwin==0) return 1;
        return 0;
    }

    if(memo[ind][lastwin][last_move]!=-1) return memo[ind][lastwin][last_move];

    int ans=0;

    int win=dp[a[ind]][ulta(last_move)]*(rec(ind+1,0,0,a,dp));
    int lose=(1-dp[a[ind]][ulta(last_move)]+mod)*rec(ind+1,1,1,a,dp);

    ans=(win+lose)%mod;

    return memo[ind][lastwin][last_move]=ans;  
...
void solve(){
    ...
    dp[0][0]=0;
    dp[0][1]=1; 
    // dp[i][j] = winning probability of alice with i stones given current moving chance is of j
    // j==0 => alice's turn
    // j==1 => bob's turn
    ...
    for(int i=1;i<=1000;i++){
        for(int j=1;j<=i;j++){
            ...
            dp[i][0]+=(1*(inv[i])*(dp[i-j][ulta(0)]));
            dp[i][0]%=mod;
            dp[i][1]+=(1*(inv[i])*(dp[i-j][ulta(1)]));
            dp[i][1]%=mod;
        }
    }
