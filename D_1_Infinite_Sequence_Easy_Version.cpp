...
int getPFX(int node,map<int,int>& dfx,int n){

  if(dfx.find(node)!=dfx.end()){
    // we know
    return dfx[node];
  }
  ...
  if(node&1){
    ...
    // 1 2 3 4 ..pfx[n] pfx[n+1].....pfx[]

    if(n&1){
      return dfx[node]=dfx[n];


    }else{
      return dfx[node]= (dfx[n]^getPFX((n+1)/2,dfx,n));

    }

    // 0 pahuchne k liye 1 aana hoga
    // then only
    ...
    if(n&1){
      return dfx[node]=(dfx[n]^getPFX(node/2,dfx,n));


    }else{
      return dfx[node]= ((dfx[n]^getPFX((n+1)/2,dfx,n))^getPFX(node/2,dfx,n));
...
void solve(){
   ...
   // dfx[0]=0;
    ...
    if(l<=n){
      cout<<a[l-1];pl; return;
    }

    cout<<getPFX(l/2,dfx,n);pl;
