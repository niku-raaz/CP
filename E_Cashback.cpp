...
void solve(){
    ...
    auto tot=[&](int left,int right)->int{

        return pfx[min(n,right+1)]-pfx[left];

    };

    auto F=[&](int ind,auto &&F)->int{
        if(ind>=n){
            return 0;
        ...
        int take=F(ind+c,F)-mn[ind]+tot(ind,ind+c-1);

        int nottake=F(ind+1,F)+a[ind];
