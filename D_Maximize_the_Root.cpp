...
// in DP check its subarray / subsequence / subset  
 ...
 bool dfs(int node,int x, int par,vector<vector<int>>& g,vector<int>& a){
    ...
    if(node!=0 && g[node].size()==1){
        ...
        // that means , for it, we must have 
        if(a[node]>=x){
            return 1;
        }else{
            return 0;
    ...
    // not leaf
    if(a[node]>=x){
        // for all its child , a[node]>=x
        bool sahil=1;
        for(auto child: g[node]){
            if(child==par){
                continue;
            }
            sahil &= dfs(child,x,node,g,a);
        }
        return sahil;

    }else{
        // a[node] < x
        // // all its child must have a[v]>=diff;
        int diff=x-a[node];
        // 
        bool rahul=1;
        ...
        for(auto child: g[node]){
            ...
            rahul &= dfs(child,x+diff,node,g,a);
        }
        return rahul;
