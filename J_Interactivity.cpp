...
void dfs(int node,int par,vector<vector<int>>& dp){
    ...
    // takes curr
    // means
    // we can use the one of the childs data
    // for all childs
    // // dp[child][0]* prod(dp[other childs][0]+[1]) 
    ...
    for(auto& child: g[node]){
        ...
        dfs(child,node,dp);
        not_takes*=(dp[child][0]+dp[child][1]);
        not_takes%=mod;
    }
    // PRE-calculation for takes

    int p=1;

    for(auto& child: g[node]){
        if(child==par) continue;
        p*=(dp[child][0]+dp[child][1]);
        p%=mod;
        pfx.push_back(p);
    }
    ...
    // lest do the calculation Now
    // takes is sum of all ways via child
    int ct=0;
