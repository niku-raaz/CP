...
double F(vector<pair<int,int>>& v, double t){
    double ans1=-2e18;
    double ans2=2e18;
    for(auto it:v){
        ans1=max(ans1,it.first*1.0+it.second*t);
        ans2=min(ans2,it.first*1.0+it.second*t);
    }
    return ans1-ans2;
}
...
void solve(){
    ...
    cout<<fixed<<showpoint<<setprecision(6)<<F(v,hi)<<endl;
