...
void dfs(int node, int par,vector<vector<int>>& g, vector<int>& dp1, vector<int>& dp2,vector<pair<int,int>>& vp){
    ...
    for(auto child: g[node]){
        ...
        dfs(child,node,g,dp1,dp2,vp);
                          //
        dp1[node]+=max(abs(vp[node].first-vp[child].first)+dp1[child],abs(vp[node].first-vp[child].second)+dp2[child]);
        dp2[node]+=max(abs(vp[node].second-vp[child].first)+dp1[child],abs(vp[node].second-vp[child].second)+dp2[child]);
  ...
  //  dp1[node]+=vp[node].first;
  //  dp2[node]+=vp[node].second;
}
...
void solve(){
    ...
    take(n);
    vector<pair<int,int>> vp(n+1),e(n);
    ...
    dfs(1,0,g,dp1,dp2,vp);
   // show(dp1);
 //   show(dp2);

    cout<<max(dp1[1],dp2[1]);pl;
