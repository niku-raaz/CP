...
int shortestSubarrayToSort(vector<int> &a) {
    int n = a.size();
    int L = 0;
    ...
    int sub_min = *min_element(a.begin()+L, a.begin()+R+1);
    int sub_max = *max_element(a.begin()+L, a.begin()+R+1);

    while (L > 0 && a[L-1] > sub_min) L--;
    while (R < n-1 && a[R+1] < sub_max) R++;
...
void solve(){
       ...
       // show(a);
        ...
        cout<<shortestSubarrayToSort(a);pl;
