...
vector<vector<int>> d(1002,vector<int>(x+1));

int rec(int i,int curr,vector<pair<int,int>>& v){
    ...
    // 
    int ans=rec(i+1,curr,v);

    if(curr>=v[i].first){
        ans=max(ans,rec(i+1,curr-v[i].first,v)+v[i].second);
