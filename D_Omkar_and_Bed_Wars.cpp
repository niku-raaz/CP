...
// int rec(int i, int prev, int prev2){
...
//     if(prev2==prev && curr==prev){
//         // all are equal
//         // 
//         // change prev2
//         int a=rec(i+1,curr,prev)+1;
//         // change prev
//         int b=rec(i+1,curr,1-prev)+1;

//         int c=rec(i+1,1-curr,prev)+1;

//         return dp[i][prev][prev2]= min(a,min(b,c));
...
//       return dp[i][prev][prev2]= rec(i+1,curr,prev);
//     }
...
int rec(int i, string& str,vector<vector<vector<int>>>& dp,int sl,int l, int f, int s){
    ...
    if(i==n-1){
        ...
        if((sl!=0 || l!=0) && (f!=0 || s!=0 ) && (l!=0 || f!=0)){
            ch=min(ch,(str[i]!='L' ? 1:0)*1LL);

        }

        if((sl!=1 || l!=1) && (f!=1 || s!=1 ) && (l!=1 || f!=1)){
            ch=min(ch,(str[i]!='R' ? 1:0)*1LL);
    ...
    if(sl!=0 || l!=0){
        ch=min(ch,(str[i]!='L'? 1: 0)+rec(i+1,str,dp,l,0,f,s));

    }

    if(sl!=1 || l!=1){

        ch=min(ch,(str[i]!='R'? 1: 0)+rec(i+1,str,dp,l,1,f,s));
   ...
   int ans=n;
    ...
    for(int i=0;i<=1;i++){
        for(int j=0;j<=1;j++){
            ...
            ans=min(ans,rec(2,s,dp,i,j,i,j)+c1+c2);
