...
int rec(int i, int currt){
    ...
    //
    int ex=0;
    if((((currt+a[i])%h)>=l) && (((currt+a[i])%h)<=r)){
        ex=1;

    }

    int t1=rec(i+1,(currt+a[i])%h)+ex;

    ex=0;
    if((((currt+a[i]-1)%h)>=l) && (((currt+a[i]-1)%h)<=r)){
        ex=1;

    }

    int t2=rec(i+1,((currt+a[i]-1)%h))+ex;

    return  dp[i][currt]= max(t1,t2);
}
   
   
void solve(){
   // take(n); take(h); take(l); take(r);
    cin>>n>>h>>l>>r;
