...
void gen(int pmask,int nmask,int ind){
    if(ind==n){
        // some condition
        // push_back
        f[pmask].push_back(nmask);
        return;
    }
    ...
    if((pmask&(1<<ind))){
        gen(pmask,nmask,ind+1);
    }else{
        // if pmask ka bit is 0
        // then
        // put horizon
        gen(pmask,(nmask|(1<<ind)),ind+1);

        // check if it is possible to do vertical

        if(ind+1<n){
            if((pmask&(1<<(ind+1)))==0){
                gen(pmask,nmask,ind+2);
            }
        }
    }
}
...
int rec(int mask,int level){
    ...
    // 
    // generate all possible case for next level
    // saare possible case mein check for valid cases

    // generate all possible next mask for current mask
    int ans=0;
    ...
    for(auto& nmask: f[mask]){
        ans+=rec(nmask,level+1);
        ans%=mod;
    }
    
    return dp[mask][level]= ans;
