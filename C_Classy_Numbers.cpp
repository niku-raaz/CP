...
int l;
string r;
// i want ki number of non-zero digits sholb less<=3

int dp[20][3][10];

int dfs(int i,bool tight,int count,string& num){
    ...
    char bound=(tight==1 ? num[i]:'9');

    
    int ans=0;

    for(char dig='0';dig<=bound;dig++){
        int newcount=count;
        if(dig!='0'){
            newcount++;
        }
        ...
        ans+=dfs(i+1,((tight==1)&&(num[i]==dig)),newcount,num);
    }
...
void solve(){
    ...
    // lest calculate for 0 to r
    // the  for 0 to l-1;
    memset(dp,-1,sizeof(dp));
    int y=dfs(0,1,0,r);
    string newl=to_string(l);
     memset(dp,-1,sizeof(dp));
    int x=dfs(0,1,0,newl);
