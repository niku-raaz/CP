...
void solve(){
    ...
    // Count number of subsegmnts st that max is x, and sum is s

    int ans=0;

    // Can do prefix sum + BS for checking sum?? No yaar a[i]'scould be negative
    // max can be done by Segment tree
    // Think for sum checking efficiently
