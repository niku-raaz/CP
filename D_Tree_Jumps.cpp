...
int DFS(int node,int par,vector<vector<int>>& g,vector<int>& d,vector<vector<int>>& level,vector<int>& dp,vector<int>& parent){
    ...
    if(node==1){
        for(auto child: g[node]){
            ...
            ans=(ans+0LL+DFS(child,node,g,d,level,dp,parent))%mod;
...
void solve(){
    ...
    vector<int> d(n+1,0),dp(n+1,-1);
    
    dfs(1,-1,g,d);

    vector<vector<int>> nodeatlevel(n+1);

    for(int i=1;i<=n;i++){
        nodeatlevel[d[i]].pb(i);
    }
    cout<<DFS(1,-1,g,d,nodeatlevel,dp,par);pl;
