...
void solve(){
    ...
    // pos-> this position is contained in those indexes
    ...
    while(start<mx){
        ...
        sort(pos[start].begin(),pos[start].end(),[&](int i,int j){
            if(g[i][start] != g[j][start]){
                return g[i][start]<g[j][start];
            }

            return lexicographical_compare(g[i].begin()+start,g[i].end(),g[j].begin()+start,g[j].end());
        ...
        for(int pp=start;pp<(int)g[taken].size();pp++){
            ans.pb(g[taken][pp]);
        }
