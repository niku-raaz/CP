...
int rec(int ind,int bound1,int bound2){
    ...
    if(dp[ind][bound1][bound2]!=-1){
        return dp[ind][bound1][bound2];
    }
    ...
    for(char digit=lo;digit<=hi;digit++){
        // if we select dig for current index
        int res=rec(ind+1,((bound1==1) && (digit==r[ind])),(bound2 && (digit==l[ind])))+(l[ind]==digit)+(r[ind]==digit);
        ans=min(ans,(res));
    }
...
void solve(){
    ...
    // Some greedy idea ? 
    // Must be digit dp
    // as each digit contribute to anwer independently
