...
int rec(int ind,int taken,vector<int>& a,vector<vector<int>>& dp){
    if(ind==a.size()) return 0;

    int mx=-1e18;

    if(dp[ind][taken]!=mx) return dp[ind][taken];

    int ans=-1e18;

    if(taken){
        ans=max(ans,rec(ind+1,0,a,dp)); // have to option to not take
    }
    ans=max(ans,a[ind]+rec(ind+1,1,a,dp)); // can always take current one
...
void solve(){
    ...
    while(lo<=hi){
        ...
        // Now I want to calculate the maximum sum subsequence
        // where at least one of the 
        // i-th and (i+1)-th cards must be chosen.
        int mx=-1e18;
        vector<vector<int>> dp(n,vector<int>(2,mx));

        if(rec(0,1,b,dp)>=0){
            ans=max(ans,(mid*1.0)/(10000));
            lo=mid+1;
        }
        else{
            hi=mid-1;
