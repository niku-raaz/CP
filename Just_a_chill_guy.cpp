...
void solve(){
    take(n); take(m); take(p);

    take(x);
    take(y);

    vector<pair<int,int>> vp(p);
    ...
    for(int i=0;i<p;i++){
        ...
        for(int j=0;j<4;j++){
            int a=vp[i].first+dx[j];
            int b=vp[i].second+dy[j];

            if(a>=1 && b>=1 && a<=n && b<=m){

                if(safe.find({a,b})!=safe.end()){
                    safe.erase({a,b});
    ...
    if(n>2 && m>2){
        ...
        for(int i=0;i<p;i++){
            int dis=abs(vp[i].first-x)+abs(vp[i].second-y);
            if(dis%2==0){
                ct++;
