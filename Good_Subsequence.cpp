...
int rec(int ind,int last,vector<int>& a,vector<vector<int>>& dp){
    if(ind==a.size()){
        return 0;
    }
    if(dp[ind][last]!=-1){
    ...
    int not_take=rec(ind+1,last,a,dp);
    int tak=0;
    if(a[ind]%2!=last){
        tak=rec(ind+1,a[ind]%2,a,dp)+1;
    }

    return dp[ind][last]= max(not_take,tak);
