...
void dfs(int node, int par, vector<vector<int>>& g, vector<int>& depth){
    if(g[node].size()>=3){
        centre=node;
    }
    if(g[node].size()==1){
...
void solve(){
    ...
    // dp[i][j] == no of ways for special tree constructions
    // such that ith node has value j

    // Good observation from hint:
    // There could be only 1 or 2 leafs
    // >=3 leafs means 0 ways;

    


    // Now remaining is Y or V shaped
    ...
    int diff= abs(depth[leafs[0]]-depth[leafs[1]]);

    int x= diff+ depth[centre]+1;

    if(diff){ 
        cout<<(binpow(2,x,mod)+binpow(2,x-1,mod))%mod;pl;
    }else{
        cout<<(2*binpow(2,x,mod))%mod;pl;
    }
