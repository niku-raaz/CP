...
void solve(){
    ...
    // leftmost
    // count number of colrs >=n

    auto lb=lower_bound(a.begin(),a.end(),n);

    int large=(m-(lb-a.begin()));
   ...
   // ans+=(large);
    ...
    for(int i=1;i<n;i++){
        ...
        int large=(m-(low-a.begin()));

        // 
        auto high=lower_bound(a.begin(),a.end(),n-i);
        int large2=(m-(high-a.begin()));

        int mn=min(large,large2);
        int mx=max(large,large2);
