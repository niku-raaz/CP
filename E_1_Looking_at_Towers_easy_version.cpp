...
int rec(int l,int r,int ld,int rd,vector<int>& next,vector<int>& prev,vector<int>& a,vector<int>& left,vector<int>& right){
    if(l==-1 || r==-1){
        return 0;
    }
    if(l==r){
    ...
    if(left[l]){
        // can take
        ans+=rec(l+1,r,1,rd,next,prev,a,left,right);
        // not take
        if(next[l]!=-1){
            if(a[next[l]]==a[l]){
                // can - not take
                ans+=rec(next[l],r,0,rd,next,prev,a,left,right);
            }
        }

    }else{
        // jab 0 hai
        // vha take or not take
        // take 
        ans+=rec(l+1,r,1,rd,next,prev,a,left,right);
        // no take
        if(prev[l]!=-1){
