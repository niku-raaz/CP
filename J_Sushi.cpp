...
double f(int n, int ones,int twos, int threes){
     ...
     // if(ones==1 && twos==0 && threes==0) return n*(1.0);
      if(dp[ones][twos][threes]!=-1.0) return dp[ones][twos][threes]; 
      ...
      if(threes>=0){
        ans+= (f(n,ones,twos+1,threes-1))*((threes*1.0)/sum);
      }
      if(twos>=0){
        ans+= (f(n,ones+1,twos-1,threes))*((twos*1.0)/sum);
      }
      if(ones>=0){
        ans+= (f(n,ones-1,twos,threes))*((ones*1.0)/sum);
      }

      // ans= ((f(n,ones,twos,threes-1)+1)*((threes*1.0)/n)+
      // (f(n,ones,twos-1,threes)+1)*((twos*1.0)/n)+
      // (f(n,ones-1,twos,threes)+1)*((ones*1.0)/n));


      return dp[ones][twos][threes] = ans;
...
void solve(){
    ...
    cout<<f(n,ones,twos,threes)<<endl;
